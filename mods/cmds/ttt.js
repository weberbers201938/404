module.exports.config = {
  ...
  cooldowns: 5,
  usages: "x/o/delete/continue <bet>"
};
...
function startBoard({isX, data, bet}) {
...
module.exports.handleReply = async function({ event, api, handleReply, Currencies }) {
  ...
  if (!isNaN(number) && number > 0 && number < 10) {
    ...
    var col = number % 3 == 0 ? 2 : (number % 3) - 1;
    ...
    if (checkGameOver(data)) {
      ...
      if (checkAIWon(data)) {
        lmao = `You lose! ${gayban[Math.floor(Math.random() * gayban.length)]}`;
        await Currencies.decreaseMoney(senderID, data.bet);
      } else if (checkPlayerWon(data)) {
        lmao = "You win! :<";
        await Currencies.increaseMoney(senderID, data.bet * 2);
      } else {
        lmao = "It's tied!";
      }
...
module.exports.run = async function({ event, api, args, Currencies }) {
  ...
  let newData;
  if (args.length < 2) return api.sendMessage("Please select X or O and specify your bet amount", threadID, messageID);
  
  let bet = parseInt(args[1]);
  if (isNaN(bet) || bet <= 0) return api.sendMessage("Invalid bet amount!", threadID, messageID);
  
  let balance = (await Currencies.getData(senderID)).money;
  if (balance < bet) return api.sendMessage("You don't have enough money to place this bet!", threadID, messageID);
  ...
  if (args[0].toLowerCase() == "continue") {
    if (!data.gameOn) return api.sendMessage("No data! use " + concak + " x/o <bet> to play new", threadID, messageID);
  ...
  if (!data.gameOn) {
    ...
    if (abc == "o") {
      newData = startBoard({ isX: false, data, bet, threadID });
    ...
    if (abc == "x") {
      newData = startBoard({ isX: true, data, bet, threadID });
      AIStart(newData);
